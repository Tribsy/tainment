"""
cogs/casino/commands.py — Casino cog: slash/prefix command bodies + adapter classes.

Phase 3a.2 step 3: extracted verbatim from cogs/casino/__init__.py. This file
holds the discord.py command dispatch surface. Game logic lives in service.py,
UI Views in views.py, constants in constants.py.

The two `_Prefix*` adapter classes wrap a prefix-command Context to behave like
a discord.Interaction object, so the same internal helpers (which expect
interaction-style API) can serve both `t!slots 100` and `/slots 100`.
"""
import asyncio
import logging
import random
...
from .constants import C, CE, ROULETTE_TABLE_PATH, WHEEL_SPIN_PATH
from .service import *  # noqa: F401, F403  (every helper from service)
from .views import BetModal, RouletteView, BlackjackView, HiLoView, DuelAcceptView

logger = logging.getLogger("tainment.casino.commands")


# ─── Adapter classes (prefix → interaction bridging) ────────────────────────
...
# ─── Setup (called by discord.py extension loader via __init__.py) ──────────
